 ...
 * Submit đặt phòng
 * 
 * DEPRECATED: Moved to BookingAjax.php (inc/frontend/AJAX/BookingAjax.php)
 * Commented out to avoid conflict with the new handler that includes Google Sheets update
 */
/*
 ...
 * 
 * DEPRECATED: Moved to BookingAjax.php (inc/frontend/AJAX/BookingAjax.php)
 * Commented out to avoid conflict with the new handler
 */
/*
 ...
 * 
 * DEPRECATED: Moved to CalendarAjax.php (inc/frontend/AJAX/CalendarAjax.php)
...
function vie_ajax_get_calendar_prices()
{
    ...
    // Nếu chỉ có hotel_id, dùng Pricing Engine để lấy giá tất cả rooms
    if ($hotel_id && class_exists('Vie_Pricing_Engine')) {
        $engine = Vie_Pricing_Engine::get_instance();
        $result = $engine->get_calendar_prices($hotel_id, $year, $month);
        wp_send_json_success($result);
    }
    wp_send_json_error(['message' => 'Cần room_id hoặc hotel_id để lấy giá']);
}
*/
 ...
 * 
 * DEPRECATED: Moved to BookingsPage.php (inc/admin/Pages/BookingsPage.php)
 * Commented out to avoid conflict
 */
/*
 ...
 * Get database status
 */
add_action('wp_ajax_vie_get_db_status', 'vie_ajax_get_db_status');

function vie_ajax_get_db_status()
{
    check_ajax_referer('vie_admin_nonce', 'nonce');
    ...
    if (!class_exists('Vie_Database_Installer')) {
        wp_send_json_error(['message' => 'Database Installer not found']);
    }

    $installer = Vie_Database_Installer::get_instance();
    $status = $installer->get_tables_status();
    ...
    wp_send_json_success([
        ...
        'latest_version' => Vie_Database_Installer::DB_VERSION
    ]);
}

/**
 * Repair/Install database
 */
add_action('wp_ajax_vie_repair_database', 'vie_ajax_repair_database');

function vie_ajax_repair_database()
{
    check_ajax_referer('vie_admin_nonce', 'nonce');
    ...
    $installer = Vie_Database_Installer::get_instance();
    $status = $installer->force_reinstall();

    wp_send_json_success([
        'message' => 'Database đã được cài đặt/sửa chữa thành công!',
        'tables' => $status,
        'version' => $installer->get_version()
    ]);
}
