...
class Vie_SePay_Webhook
{
    ...
    private function maybe_send_confirmation_email($booking, $payment_status, $amount)
    {
        ...
        $subject = sprintf(
            '[%s] Xác nhận thanh toán đặt phòng #%s',
            get_bloginfo('name'),
        ...
        $message = sprintf(
            "Xin chào %s,\n\n" .
            "Chúng tôi đã nhận được thanh toán của bạn cho đơn đặt phòng #%s.\n\n" .
            "Thông tin đặt phòng:\n" .
            "- Khách sạn: %s\n" .
            "- Loại phòng: %s\n" .
            "- Ngày nhận phòng: %s\n" .
            ...
            $hotel_name,
            $room_name,
            date_i18n('d/m/Y', strtotime($booking->check_in)),
            date_i18n('d/m/Y', strtotime($booking->check_out)),
            $booking->num_rooms,
            $this->sepay->format_currency($booking->total_amount),
            $this->sepay->format_currency($amount),
            get_bloginfo('name')
        );
