...
class Vie_Hotel_Rooms_Helpers
{
    ...
    public static function sanitize_room_data($data)
    {
        ...
        return array(
            ...
            'price_includes' => sanitize_textarea_field($data['price_includes'] ?? ''),
            'cancellation_policy' => wp_kses_post($data['cancellation_policy'] ?? ''),
     ...
     * 
     * Maps booking status slug to Vietnamese label and color badge
     * 
     * @param string $status Status slug
     * @param bool $html_output Return HTML badge or just array
     * @return mixed Array with label/color or HTML string
     */
    public static function get_booking_status_badge($status, $html_output = true)
    {
        $status_map = array(
            'pending_payment' => array(
                ...
                'bg_color' => '#fef3c7',
                'css_class' => 'vie-badge-warning'
            ),
            'processing' => array(
                'label' => 'Đã thanh toán (Chờ xác nhận)',
                'color' => '#3b82f6', // Blue
                'bg_color' => '#dbeafe',
                'css_class' => 'vie-badge-info'
            ),
            'confirmed' => array(
                'label' => 'Đã xác nhận',
                'color' => '#10b981', // Green
                'bg_color' => '#d1fae5',
                'css_class' => 'vie-badge-success'
            ),
            'cancelled' => array(
                'label' => 'Đã hủy',
                'color' => '#6b7280', // Gray/Black
                'bg_color' => '#f3f4f6',
                'css_class' => 'vie-badge-default'
            ),
            'completed' => array(
                'label' => 'Hoàn tất',
                'color' => '#8b5cf6', // Purple
                'bg_color' => '#ede9fe',
                'css_class' => 'vie-badge-purple'
            ),
            // Legacy statuses
            'pending' => array(
                'label' => 'Chờ xác nhận',
            ...
            'no_show' => array(
                ...
                'color' => '#ef4444',
                'bg_color' => '#fee2e2',
                'css_class' => 'vie-badge-danger'
            )
        );

        $status_info = isset($status_map[$status])
            ? $status_map[$status]
            : array(
        ...
        // Return HTML badge
        return sprintf(
            '<span class="vie-booking-status-badge %s" style="background-color: %s; color: %s;">%s</span>',
            esc_attr($status_info['css_class']),
            esc_attr($status_info['bg_color']),
