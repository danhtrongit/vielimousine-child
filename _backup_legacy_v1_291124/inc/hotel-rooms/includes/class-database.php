...
class Vie_Hotel_Rooms_Database
{
    ...
    public static function create_tables()
    {
        ...
        $sql_rooms = "CREATE TABLE {$table_rooms} (
            ...
            price_includes TEXT COMMENT 'Giá bao gồm gì - VD: Ăn sáng, Wifi, Spa',
            cancellation_policy LONGTEXT COMMENT 'Chính sách hủy phòng riêng',
        ...
        $sql_bookings = "CREATE TABLE {$table_bookings} (
            ...
            room_code VARCHAR(50) DEFAULT NULL COMMENT 'Mã nhận phòng thực tế từ khách sạn',
            ...
            invoice_info LONGTEXT COMMENT 'JSON: Thông tin xuất hóa đơn',
    ...
    /**
     * Migrate room fields - Add price_includes and cancellation_policy
     * Version 1.1.0 update
     */
    public static function migrate_room_fields()
    {
        global $wpdb;
        ...
        // Check if price_includes column exists
        $price_includes_exists = $wpdb->get_results("SHOW COLUMNS FROM {$table} LIKE 'price_includes'");

        if (empty($price_includes_exists)) {
            $wpdb->query("
                ALTER TABLE {$table} 
                ADD COLUMN price_includes TEXT COMMENT 'Giá bao gồm gì - VD: Ăn sáng, Wifi, Spa'
                AFTER base_price
            ");
            $changes[] = 'price_includes';
            error_log('Vie Hotel Rooms: Added column price_includes to wp_hotel_rooms');
        }

        // Check if cancellation_policy column exists
        $cancellation_policy_exists = $wpdb->get_results("SHOW COLUMNS FROM {$table} LIKE 'cancellation_policy'");

        if (empty($cancellation_policy_exists)) {
            $wpdb->query("
                ALTER TABLE {$table} 
                ADD COLUMN cancellation_policy LONGTEXT COMMENT 'Chính sách hủy phòng riêng'
                AFTER price_includes
            ");
            $changes[] = 'cancellation_policy';
            error_log('Vie Hotel Rooms: Added column cancellation_policy to wp_hotel_rooms');
        }
