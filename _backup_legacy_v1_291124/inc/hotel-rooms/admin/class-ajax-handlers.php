...
class Vie_Hotel_Rooms_Ajax
{
    ...
    private function __construct()
    {
        ...
        // Price Matrix Grid
        add_action('wp_ajax_vie_get_matrix_data', array($this, 'get_matrix_data'));
        add_action('wp_ajax_vie_save_matrix_data', array($this, 'save_matrix_data'));

        // Booking Actions
        add_action('wp_ajax_vie_update_room_code', array($this, 'update_room_code'));
    }
    ...
    public function bulk_update_pricing()
    {
        ...
        while ($current <= $end) {
            ...
            if (
                $rule['price_room'] !== null || $rule['price_combo'] !== null
    ...
    /**
     * Get matrix data for Price Matrix Grid
     * 
     * Loads all hotels, their rooms, and pricing data for a specific month
     * Optimized for large data sets with proper indexing
     */
    public function get_matrix_data()
    {
        $this->verify_nonce();
        ...
        if ($hotel_id > 0) {
            $hotels_args['p'] = $hotel_id;
        }

        $hotels_query = get_posts($hotels_args);

        if (empty($hotels_query)) {
            wp_send_json_error(array('message' => 'Không tìm thấy khách sạn nào'));
        }
        ...
        foreach ($hotels_query as $hotel) {
            ...
            $rooms = $wpdb->get_results($wpdb->prepare(
                "SELECT id, name, total_rooms, base_price, base_occupancy 
                 FROM {$table_rooms} 
            ...
            // Get room IDs for pricing query
            $room_ids = wp_list_pluck($rooms, 'id');
            $room_ids_str = implode(',', array_map('intval', $room_ids));

            // Get pricing data for all rooms in date range
            $pricing = $wpdb->get_results($wpdb->prepare(
                "SELECT room_id, date, price_room, price_combo, stock, status 
                 FROM {$table_pricing} 
            ...
            // Index pricing by room_id
            $pricing_by_room = array();
            foreach ($pricing as $p) {
                if (!isset($pricing_by_room[$p->room_id])) {
                    $pricing_by_room[$p->room_id] = array();
                }
                $pricing_by_room[$p->room_id][] = array(
                    'date' => $p->date,
                    'price_room' => $p->price_room ? floatval($p->price_room) : null,
                    'price_combo' => $p->price_combo ? floatval($p->price_combo) : null,
                    'stock' => intval($p->stock),
                    'status' => $p->status
                );
            }

            // Build rooms data with pricing
            $rooms_data = array();
            foreach ($rooms as $room) {
                $rooms_data[] = array(
                    ...
                    'base_occupancy' => intval($room->base_occupancy),
                    'pricing' => isset($pricing_by_room[$room->id]) ? $pricing_by_room[$room->id] : array()
                );
            }
    ...
    /**
     * Save matrix data (bulk update)
     * 
     * Receives array of changes and processes them in batches
     * Uses INSERT ... ON DUPLICATE KEY UPDATE for optimal performance
     */
    public function save_matrix_data()
    {
        $this->verify_nonce();
        ...
        if (empty($changes)) {
            wp_send_json_error(array('message' => 'Không có dữ liệu để lưu'));
        }

        // Validate and prepare batch data
        $batch_data = array();
        $room_cache = array(); // Cache room data to avoid repeated queries

        foreach ($changes as $change) {
            $room_id = isset($change['room_id']) ? absint($change['room_id']) : 0;
            $date = isset($change['date']) ? sanitize_text_field($change['date']) : '';
            ...
            // Get room info (cached)
            if (!isset($room_cache[$room_id])) {
            ...
            $day_of_week = intval($date_obj->format('w')); // 0=Sunday, 6=Saturday

            $price_combo = isset($change['price_combo']) && $change['price_combo'] !== ''
                ? floatval($change['price_combo']) : null;
            $price_room = isset($change['price_room']) && $change['price_room'] !== ''
                ? floatval($change['price_room']) : null;
            $stock = isset($change['stock']) && $change['stock'] !== ''
                ? absint($change['stock']) : $room->total_rooms;
        ...
        if (empty($batch_data)) {
            wp_send_json_error(array('message' => 'Không có dữ liệu hợp lệ để lưu'));
        }
        ...
        foreach ($batches as $batch) {
            ...
            // Use COALESCE to preserve existing values when new value is 0
            $sql = "INSERT INTO {$table_pricing} (room_id, date, day_of_week, price_room, price_combo, stock, status) 
                    VALUES " . implode(', ', $placeholders) . "
                    ON DUPLICATE KEY UPDATE 
                        price_room = CASE WHEN VALUES(price_room) > 0 THEN VALUES(price_room) 
                                          WHEN VALUES(price_room) = 0 AND price_room > 0 THEN price_room 
                                          ELSE VALUES(price_room) END,
                        price_combo = CASE WHEN VALUES(price_combo) > 0 THEN VALUES(price_combo) 
                                           WHEN VALUES(price_combo) = 0 AND price_combo > 0 THEN price_combo 
                                           ELSE VALUES(price_combo) END,
                        stock = VALUES(stock),
                        status = VALUES(status)";
        ...
        wp_send_json_success(array(
            'message' => sprintf('Đã lưu %d thay đổi thành công', count($batch_data)),
            'total_changes' => count($batch_data),
            'affected_rows' => $total_affected
        ));
    }
    /**
     * Update room code manually
     */
    public function update_room_code()
    {
        $this->verify_nonce();
        ...
        if (empty($room_code)) {
            wp_send_json_error(array('message' => 'Vui lòng nhập mã nhận phòng'));
        }

        global $wpdb;
        $table = $wpdb->prefix . 'hotel_bookings';

        // Update room_code and status
        $updated = $wpdb->update(
            $table,
            array(
                'room_code' => $room_code,
        ...
        if ($updated === false) {
            wp_send_json_error(array('message' => 'Lỗi cập nhật database: ' . $wpdb->last_error));
        }

        // Send Email 3 (Completed)
        if (class_exists('Vie_Hotel_Rooms_Email_Manager')) {
            Vie_Hotel_Rooms_Email_Manager::get_instance()->send_email('completed', $booking_id);
        }

        wp_send_json_success(array('message' => 'Đã cập nhật mã phòng và gửi email thành công!'));
    }
}
