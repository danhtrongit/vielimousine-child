...
class Vie_Hotel_Rooms_Admin_Bookings {
    ...
    private function render_bookings_list() {
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if (!empty($booking->invoice_info)) : ?>
                                <span class="dashicons dashicons-media-spreadsheet" title="<?php _e('Yêu cầu xuất hóa đơn', 'flavor'); ?>" style="color:#dc2626; font-size:16px; width:16px; height:16px; vertical-align:middle;"></span>
                            <?php endif; ?>
                            <br>
                        ...
                        <td>
                            <?php echo Vie_Hotel_Rooms_Helpers::get_booking_status_badge($booking->status); ?>
                        </td>
                        ...
                        <td>
                            ...
                            <button type="button" class="button button-small button-primary vie-update-room-code" 
                                    ...
                                    title="<?php _e('Cập nhật Mã nhận phòng', 'flavor'); ?>">
                                <span class="dashicons dashicons-tickets-alt" style="line-height:1.3"></span>
                            </button>
                            <?php endif; ?>
                        </td>
                    </tr>
        ...
        <!-- Room Code Modal (WordPress Native Style) -->
        <div id="vie-room-code-modal" class="vie-modal" style="display:none;">
            <div class="vie-modal-box">
                <div class="vie-modal-header">
                    <h3><?php _e('Cập nhật Mã nhận phòng', 'flavor'); ?></h3>
                    <button type="button" class="vie-modal-close" aria-label="<?php _e('Đóng', 'flavor'); ?>">
                        <span class="dashicons dashicons-no-alt"></span>
                    </button>
                </div>
                <div class="vie-modal-body">
                    <p><?php _e('Nhập mã xác nhận từ khách sạn để gửi cho khách hàng.', 'flavor'); ?></p>
                    
                    <p>
                        <strong><?php _e('Lưu ý:', 'flavor'); ?></strong> 
                        <?php _e('Hệ thống sẽ tự động gửi Email chứa mã này cho khách và chuyển trạng thái đơn sang "Hoàn thành".', 'flavor'); ?>
                    </p>
                    
                    <div class="form-field">
                        <label for="vie-room-code-input"><?php _e('Mã nhận phòng:', 'flavor'); ?></label>
                        <input type="text" id="vie-room-code-input" class="regular-text" placeholder="VD: CONF-123456">
                    </div>
                    
                    <input type="hidden" id="vie-room-code-booking-id">
                </div>
                <div class="vie-modal-footer">
                    <button type="button" class="button vie-modal-close"><?php _e('Hủy', 'flavor'); ?></button>
                    <button type="button" id="vie-save-room-code" class="button button-primary"><?php _e('Lưu & Gửi Email', 'flavor'); ?></button>
                </div>
            </div>
        </div>
        ...
        jQuery(function($) {
            ...
            $('#vie-save-room-code').on('click', function() {
                ...
                if (!code) {
                    alert('<?php _e('Vui lòng nhập mã nhận phòng', 'flavor'); ?>');
                    return;
                }
                
                $btn.prop('disabled', true).text('<?php _e('Đang xử lý...', 'flavor'); ?>');
                ...
                $.post(ajaxurl, {
                    ...
                    if (res.success) {
                        ...
                        alert(res.data.message);
                        $btn.prop('disabled', false).text('<?php _e('Lưu & Gửi Email', 'flavor'); ?>');
                    }
                });
            });
        });
            ...
            <div id="poststuff">
                <div id="post-body" class="metabox-holder columns-2">
                    ...
                    <div id="post-body-content">
                        ...
                        <div class="postbox">
                            ...
                            <div class="inside">
                                <table class="form-table">
                                    ...
                                    <?php endif; ?>
                                    <tr>
                                        <th><?php _e('Loại giường', 'flavor'); ?></th>
                                        <td>
                                            <span class="vie-bed-type-badge">
                        ...
                        <!-- Invoice Info -->
                        <?php 
                        $invoice_info = json_decode($booking->invoice_info ?? '', true);
                        if (!empty($invoice_info)) : 
                        ?>
                        <div class="postbox vie-invoice-box">
                            <div class="postbox-header">
                                <h2>
                                    <span class="dashicons dashicons-media-spreadsheet" style="color:#dc2626"></span>
                                    <?php _e('Thông tin xuất hóa đơn (VAT)', 'flavor'); ?>
                                </h2>
                            </div>
                            <div class="inside">
                                <table class="form-table">
                                    <tr>
                                        <th><?php _e('Tên công ty', 'flavor'); ?></th>
                                        <td><strong><?php echo esc_html($invoice_info['company_name'] ?? ''); ?></strong></td>
                                    </tr>
                                    <tr>
                                        <th><?php _e('Mã số thuế', 'flavor'); ?></th>
                                        <td><?php echo esc_html($invoice_info['tax_id'] ?? ''); ?></td>
                                    </tr>
                                    <tr>
                                        <th><?php _e('Email nhận hóa đơn', 'flavor'); ?></th>
                                        <td>
