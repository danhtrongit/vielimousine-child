...
class Vie_Hotel_Rooms_Admin
{
    ...
    public function add_admin_menu()
    {
        ...
        // Submenu - Bulk Update (Price Matrix)
        add_submenu_page(
            'vie-hotel-rooms',
    ...
    public function enqueue_admin_scripts($hook)
    {
        ...
        // Load only on plugin pages or hotel post type
        if (
            strpos($hook, 'vie-hotel-rooms') === false &&
            $post_type !== 'hotel' &&
            $current_page !== 'vie-hotel-bookings'
        ) {
            return;
        }

        // WordPress dependencies (always load when on our pages)
        wp_enqueue_media();
        ...
        // ALWAYS LOAD: Variables (design system)
        wp_enqueue_style(
            'vie-hotel-variables',
            VIE_HOTEL_ROOMS_URL . '/assets/admin/css/_variables.css',
            array(),
            VIE_HOTEL_ROOMS_VERSION
        );

        // ALWAYS LOAD: Common styles (shared across all pages)
        wp_enqueue_style(
            'vie-hotel-common-css',
            VIE_HOTEL_ROOMS_URL . '/assets/admin/css/common.css',
            array('vie-hotel-variables'),
            VIE_HOTEL_ROOMS_VERSION
        );

        // ALWAYS LOAD: Common JavaScript
        wp_enqueue_script(
            'vie-hotel-common-js',
            VIE_HOTEL_ROOMS_URL . '/assets/admin/js/common.js',
            array('jquery'),
            VIE_HOTEL_ROOMS_VERSION,
            true
        );

        // PAGE-SPECIFIC: Bookings page
        if ($current_page === 'vie-hotel-bookings') {
            wp_enqueue_style(
                'vie-hotel-bookings-css',
                VIE_HOTEL_ROOMS_URL . '/assets/admin/css/page-bookings.css',
                array('vie-hotel-common-css'),
                VIE_HOTEL_ROOMS_VERSION
            );

            wp_enqueue_script(
                'vie-hotel-bookings-js',
                VIE_HOTEL_ROOMS_URL . '/assets/admin/js/page-bookings.js',
                array('jquery', 'vie-hotel-common-js'),
                VIE_HOTEL_ROOMS_VERSION,
                true
            );
        }

        // PAGE-SPECIFIC: Rooms management pages
        if (strpos($hook, 'vie-hotel-rooms') !== false && $current_page !== 'vie-hotel-bookings') {
            wp_enqueue_style(
                'vie-hotel-rooms-css',
                VIE_HOTEL_ROOMS_URL . '/assets/admin/css/page-rooms.css',
                array('vie-hotel-common-css'),
                VIE_HOTEL_ROOMS_VERSION
            );
            ...
            wp_enqueue_script(
                ...
                VIE_HOTEL_ROOMS_URL . '/assets/admin/js/page-rooms.js',
                array('jquery', 'jquery-ui-datepicker', 'vie-hotel-common-js'),
                VIE_HOTEL_ROOMS_VERSION,
                true
            );
        }

        // PAGE-SPECIFIC: Calendar page
        if (strpos($hook, 'calendar') !== false || strpos($current_page, 'calendar') !== false) {
            ...
            wp_enqueue_script(
                'vie-hotel-rooms-calendar',
                VIE_HOTEL_ROOMS_URL . '/assets/admin/js/page-calendar.js',
                array('jquery', 'fullcalendar-js', 'vie-hotel-common-js'),
                VIE_HOTEL_ROOMS_VERSION,
                true
            );
        }

        // PAGE-SPECIFIC: Bulk update/matrix page
        if (strpos($hook, 'bulk') !== false || strpos($current_page, 'bulk') !== false) {
            // Matrix grid styles (complex sticky table)
            wp_enqueue_style(
                'vie-hotel-matrix-css',
                VIE_HOTEL_ROOMS_URL . '/assets/admin/css/page-bulk-matrix.css',
                array('vie-hotel-common-css'),
                VIE_HOTEL_ROOMS_VERSION
            );

            wp_enqueue_script(
                'vie-hotel-rooms-matrix',
                VIE_HOTEL_ROOMS_URL . '/assets/admin/js/page-bulk-matrix.js',
                array('jquery', 'vie-hotel-common-js'),
                VIE_HOTEL_ROOMS_VERSION,
                true
            );
        ...
        $localize_handle = 'vie-hotel-common-js';
        if ($current_page === 'vie-hotel-bookings') {
            $localize_handle = 'vie-hotel-bookings-js';
        } elseif (wp_script_is('vie-hotel-rooms-admin', 'enqueued')) {
            $localize_handle = 'vie-hotel-rooms-admin';
        }

        wp_localize_script($localize_handle, 'vieHotelRooms', array(
    ...
    /**
     * Render price matrix page (Bulk Update - All-in-One Grid)
     */
    public function render_matrix_page()
    {
        $hotels = Vie_Hotel_Rooms_Helpers::get_hotels();

        include VIE_HOTEL_ROOMS_PATH . '/admin/views/price-matrix.php';
    }
}
