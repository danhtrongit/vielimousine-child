...
class Vie_Hotel_Rooms_Frontend_Ajax
{
    ...
    public function submit_booking()
    {
        ...
        $data = array(
            ...
            'bed_type' => sanitize_text_field($_POST['bed_type'] ?? 'double'), // Bed type preference
        ...
        // Prepare guests info JSON (includes bed_type preference)
        $guests_info = array(
            'adults' => $data['num_adults'],
            'children' => $data['num_children'],
            'children_ages' => $data['children_ages'],
            'rooms_allocation' => $data['num_rooms'],
            'bed_type' => $data['bed_type'], // 'double' or 'twin'
            'bed_type_label' => $data['bed_type'] === 'twin' ? '2 Giường đơn (Twin Beds)' : '1 Giường đôi lớn (Double Bed)'
        );
        ...
        $insert_data = array(
            ...
            'invoice_info' => $data['invoice_info'] ? wp_json_encode($data['invoice_info']) : null, // New: Invoice info
    ...
    public function process_checkout()
    {
        ...
        // Send Processing Email (Email 2)
        if (class_exists('Vie_Hotel_Rooms_Email_Manager')) {
            $email_manager = Vie_Hotel_Rooms_Email_Manager::get_instance();
            $email_manager->send_email('processing', $booking->id);
        }
